...
Real-time dashboard for the RP3 rowing machine — tkinter + matplotlib version.

Same program as the PySide6 one, same architecture, no Qt anywhere:

    [ socket thread ]  ->  [ queue.Queue ]  ->  [ tkinter main loop ]
     reads TCP bytes        thread-safe          root.after() drains the
     splits on "\\n"        hand-off             queue and redraws

  * StrokeReader runs in a background thread. It owns the socket, the newline
    buffering, the JSON parsing, reconnection and the CSV file. It never
    touches a widget.
  * Everything it produces goes onto a queue.Queue as small tuples.
  * The window polls that queue 25 times a second from the main thread, which
    is the only thread allowed to touch tkinter.

Only matplotlib needs installing; tkinter ships with Python (on Debian/Ubuntu:
sudo apt install python3-tk).

Run `python rp3_dashboard_tk.py --help` for the options.
"""
...
# --------------------------------------------------------------------------
class Dashboard:
    def __init__(self, root: tk.Tk, source_description: str):
        self.root = root
...
def main() -> int:
    ...
    parser.add_argument("--port", type=int, default=56652, help="RP3 TCP port")
    ...
    if not args.no_csv:
        ...
        if path is None:
            ...
            path = os.path.join("logs", time.strftime("rp3_%Y-%m-%d_%H-%M-%S.csv"))
